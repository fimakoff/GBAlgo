...
public class MyArrayList<Item> implements Iterable<Item> {
    ...
    public int indexOf(Item item) {
        for (int i = 0; i < size; i++) if (list[i].equals(item)) return i;
        return -1;
    }
    ...
    public void bubbleSort(Comparator<Item> cmp) {
        for (int i = size - 1; i >= 1; i--) {
            for (int j = 0; j < i; j++) {
                if (more((Item) list[j], (Item) list[j + 1], cmp)) {
                    exch(i, i + 1);
                }
            }
        }
    ...
    private boolean more(Item item1, Item item2, Comparator<Item> cmp) {
        return cmp.compare(item1, item2) > 0;
    }
}
